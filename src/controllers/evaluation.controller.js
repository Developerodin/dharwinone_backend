...
import * as evaluationQuizGradeService from '../services/evaluationQuizGrade.service.js';
...
/**
 * GET quiz attempts for a student on a course (trainer).
 */
const listStudentQuizAttempts = catchAsync(async (req, res) => {
  const { studentId, moduleId } = req.params;
  const result = await evaluationQuizGradeService.listStudentQuizAttempts(studentId, moduleId);
  res.send(result);
});
 ...
 */
const gradeQuizAttempt = catchAsync(async (req, res) => {
  const reviewerId = req.user.id || req.user._id;
  const result = await evaluationQuizGradeService.gradeQuizAttemptByTrainer(
    req.params.attemptId,
    reviewerId,
    req.body
  );

 ...
 *
 * HOW (faithful to runtime — no re-implementation of the rules):
 ...
 *      intersects the row's granted set — exactly the check hasApiPermissionFromContext does.
 *
 ...
 * BUG: ... entries are tracked defects (wrong-key gating), not "fine" — fix or remove the row.
 */
const INTENTIONALLY_UNENFORCED = {
  ...
  'training.courses': 'BUG: namespaced training-courses.* derived but no route consumes it; admin training-course APIs gate on students.courses.*/modules.* — wire training-courses.* or remove the row.',
  ...
  'ai.chatbot': 'chatAssistant routes are auth-only — wire chatbot.* or remove the row.',
};
 ...
 * (a dead route only platformSuperUser can reach) — e.g. recruiters.update was one
...
function parseMatrixRows(tsSource) {
  ...
  if (!rows.length) throw new Error('Parsed 0 matrix rows — regex drift, check roles-permissions.ts shape');
  return rows;
}
 ...
 * the guard at runtime) intersects the row's granted set — identical to the runtime check.
 */
...
function auditMatrix() {
  ...
  // A guarded key no row can grant is a wrong-key bug — a route only platformSuperUser
  ...
  // guarded, or became satisfiable (someone aliased/derived it) — delete it then.
...
function main() {
  ...
  if (r.dead.length) {
    ...
    for (const row of r.dead) {
      ...
      console.log(`  ${reason ? '•' : '✗'} ${row.prefix}  ${reason ? `(allowlisted: ${reason})` : '<-- UNEXPECTED'}`);
    }
    console.log('');
  }
  ...
  if (r.staleAllowlist.length) {
    fail = true;
    console.error('FAIL: INTENTIONALLY_UNENFORCED entries that are now ENFORCED (stale — delete them):');
  ...
  if (r.unsatisfiableGuards.length) {
    fail = true;
    console.error('FAIL: routes guard on permission keys NO matrix row can grant (wrong-key — only platformSuperUser passes):');
  ...
  if (r.staleRestricted.length) {
    fail = true;
    console.error('FAIL: KNOWN_RESTRICTED_KEYS entries no longer unsatisfiable (stale — delete them):');

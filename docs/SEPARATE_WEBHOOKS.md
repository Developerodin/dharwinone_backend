...
For local testing with [ngrok](https://ngrok.com) (or similar), replace **`YOUR_NGROK_HOST`** below with the hostname ngrok assigns (the subdomain before `.ngrok-free.dev`).
...
**Local (ngrok):**
```
https://YOUR_NGROK_HOST.ngrok-free.dev/api/v1/webhooks/bolna-calls
```
...
**Local (ngrok):**
```
https://YOUR_NGROK_HOST.ngrok-free.dev/api/v1/webhooks/bolna-candidate-calls
```

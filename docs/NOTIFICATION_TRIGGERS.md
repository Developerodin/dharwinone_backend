...
| Meeting reminder (T-15) | `meeting.service.js` `sendUpcomingMeetingReminders` | `notify` + email | `meeting_reminder` | Users matching invite emails + guest emails | Email to any invite address; in-app when User exists | `Meeting.reminderSentAt` + `reminderRetry` lease in MongoDB; window `[now+15, now+25]` min (env `REMINDER_WINDOW_START_MIN` / `REMINDER_WINDOW_END_MIN`), 5-min scheduler tick |
| Internal meeting invite | `internalMeeting.service.js` create / new invitee / resend | `notifyByEmail` | `meeting` | Each invite email (deduped) | No in-app for guests without User | Emails from `getInvitationEmails`; recurring series sends per occurrence |
| Internal meeting reminder | `internalMeeting.service.js` `sendUpcomingInternalMeetingReminders` | `notify` + email | `meeting_reminder` | Users matching invite emails; guest emails without User | Email-only for guests | `InternalMeeting.reminderState.<minutes>` per `INTERNAL_MEETING_REMINDER_WINDOWS` (default 60, 15); �`INTERNAL_MEETING_REMINDER_PAD_MIN` window; in-app + email for users, email-only for guests |
